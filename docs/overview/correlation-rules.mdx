...
|----------|-------------|
| **New incident** created by the rule | Templates rendered from the **triggering alert** and saved |
| **Existing incident** reused (same rule fingerprint) | **Not re-run** — enrichments stay from the first alert that created the incident |

Enrichments are resolved from the **single alert that triggered incident creation** in that request — not from alerts that are added later.

Data for templates comes from:
